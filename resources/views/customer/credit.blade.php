                        ...
                        <div class="ui segment">
                          ...
                          <div class="ui bottom attached tab segment" data-tab="second">
                            ...
                            @if ($paybacks->lastPage() > 1)
                                <div class="ui pagination menu">
                                    <a href="{{ $paybacks->previousPageUrl() }}" class="{{ ($paybacks->currentPage() == 1) ? ' disabled' : '' }} item">
                                        Previous
                                    </a>
                                    @for ($i = 1; $i <= $paybacks->lastPage(); $i++)
                                        <a href="{{ $paybacks->url($i) }}" class="{{ ($paybacks->currentPage() == $i) ? ' active' : '' }} item">
                                            {{ $i }}
                                        </a>
                                    @endfor
                                    <a href="{{ $paybacks->nextPageUrl() }}" class="{{ ($paybacks->currentPage() == $paybacks->lastPage()) ? ' disabled' : '' }} item">
                                        Next
                                    </a>
                                </div>
                            @endif
